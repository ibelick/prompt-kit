...
ResponseStream can be combined with the Markdown component to create rich, animated content.
...
### Using the useTextStream Hook

For more control over the streaming behavior, you can use the `useTextStream` hook directly.
Note: When using the useTextStream hook with `fade` mode, you need to manually render the segments with appropriate CSS animations.
