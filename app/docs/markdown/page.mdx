...
**Recommendation**: For the best typography styling experience, install the [@tailwindcss/typography](https://github.com/tailwindlabs/tailwindcss-typography) plugin. This provides beautiful typographic defaults through the `prose` classes.

<CodeBlock code={`npm install -D @tailwindcss/typography`} language="bash" />

See the [Tailwind Typography](#styling-with-tailwind-typography) section for usage details.
