...
const PROMPT_KIT_REGISTRY_URL = "https://raw.githubusercontent.com/ibelick/prompt-kit/main/public/c/registry.json";
const PROMPT_KIT_BASE_URL = "https://raw.githubusercontent.com/ibelick/prompt-kit/main/";
const TARGET_DIR = "components/prompt-kit";
...
program
  ...
  .action(async (component: string) => {
    ...
    try {
      // Fetch the prompt-kit registry
      const promptKitRegistry = await fetchRegistry(PROMPT_KIT_REGISTRY_URL);
      const componentEntry = promptKitRegistry.items.find(
        (item) => item.name === component
      );
      if (!componentEntry) {
        spinner.fail(`Component "${component}" not found in prompt-kit registry`);
        process.exit(1);
      }

      // Collect all files and dependencies
      const allFiles: { path: string; content: string }[] = [];
      const allDependencies: Set<string> = new Set(componentEntry.dependencies || []);

      // Process all files from registry.json
      for (const file of componentEntry.files) {
        const content = file.content || (await fetchFile(`${PROMPT_KIT_BASE_URL}${file.path}`));
      ...
      if (depsArray.length > 0) {
        ...
        spinner.succeed(`Component "${component}" added successfully! No additional dependencies needed.`);
      }
    } catch (error: any) {
